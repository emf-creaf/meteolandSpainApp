...
La aplicación **Meteoland App** ofrece estimaciones meteorológicas diarias para toda España, correspondientes al último año natural, basadas en datos de estaciones meteorológicas. Este servicio está diseñado para apoyar estudios que requieren información climática diaria, complementando los datos que ofrecen las instituciones mencionadas. Es importante recordar que los datos proporcionados incluyen interpolaciones y cálculos basados ​​en modelos, por lo que los valores resultantes pueden presentar diferencias significativas con respecto a las mediciones.
...
Es importante señalar que hay variables que no se interpolan, sino que se calculan *a posteriori* utilizando las variables previamente interpoladas como predictores:
...
Como se mencionó anteriormente, la metodología de interpolación requiere especificar los parámetros $\alpha$ y $N$ para cada variable a interpolar (en el caso de la precipitación, se trata de dos pares). Estos parámetros se estimaron mediante calibraciones para cada día del año en curso, determinando aquellos que minimizaron el error de estimación en las mismas estaciones base.
La calibración no se realizó para toda el área objetivo. En su lugar, el área a interpolar se dividió en cuadrantes que contienen al menos 45 estaciones meteorológicas para evitar áreas sin cobertura e incluyen una zona de amortiguamiento para evitar efectos de borde. Los parámetros se calibran para cada uno de estos cuadrantes por separado.

### Rendimiento predictivo

El rendimiento predictivo de la metodología de estimación con los parámetros calibrados se puede consultar en la pestaña "Validaciones cruzadas". Dependiendo de la variable, las estadísticas de rendimiento incluyen el número de estaciones utilizadas, el sesgo absoluto o relativo, el error absoluto medio o el coeficiente de determinación. Se proporciona el rendimiento predictivo para cada uno de los cuadrantes utilizados en la calibración de parámetros.
