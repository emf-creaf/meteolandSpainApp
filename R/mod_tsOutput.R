...
mod_tsOutput <- function(id) {
  ...
  ns <- shiny::NS(id)
  shiny::tagList(
    echarts4r::echarts4rOutput(ns("output_ts"), height = 400)
  )
}
...
mod_ts <- function(
  ...
  ts_data <- shiny::reactive({
    ...
    ts_query <- glue::glue(
      ...
      WHERE geom.x > {purrr::map_dbl(coords_bbox, 'xmin')} AND
        geom.x < {purrr::map_dbl(coords_bbox, 'xmax')} AND
        geom.y > {purrr::map_dbl(coords_bbox, 'ymin')} AND
        geom.y < {purrr::map_dbl(coords_bbox, 'ymax')}
      GROUP BY dates;"
    )
  ...
  output$output_ts <- echarts4r::renderEcharts4r({
    # get the data
    ts_data_sel <- ts_data()

    ts_data_sel |>
      echarts4r::e_charts(dates) |>
      echarts4r::e_line(MeanTemperature) |>
      echarts4r::e_area(MinTemperature) |>
      echarts4r::e_title("Testing echarts4r") |>
